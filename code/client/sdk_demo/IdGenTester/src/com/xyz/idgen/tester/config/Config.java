package com.xyz.idgen.tester.config;
...
/**
 * <br>Config 类，singleton。使用方法： 
 * <br>1、初始化：Config.getInstance().loadConfig();
 * <br>2、读取：Config.getInstance().getXXX(); 
 * <br>3、保存：Config.getInstance().save();
 * 
 * @author 
 */
public class Config
{
	...
	public boolean loadConfig(String configFile)
	{
		...
		try{
			load(new FileInputStream(configFile));	
			 ...
			 m_logger.error("exception:Load configure file: " + configFile + " fail,detail informaiton: " + e);
			 return false;
		}
		return true; 
	...
	/*	ArrayList<Config.Property> list = Db.loadConfig(ConfigKey.DB_CONFIG_CATEGORY_ALL);
	...
	private void put(String key, String value)
	{
		...
		{
			pl = new Property(String.format("%s = %s", key, value), m_contents.size());
			m_contents.add(pl);
			m_nvmap.put(key, pl);
		}
	}
	...
	/**
	 * 更新�?�? key - value 对，会保�? value.toString() 的�??
	 * 
	 * @param key
	 * @param value
	 */
	...
	/**
	 * 设置�?�? key - value 对，会保�? value.toString() 的�??
	 * 
	 * @param key
	 * @param value
	 */
	...
	/**
	 * 更新�?�? boolean 值�?�实际写到配置中时，采用的是整数：true - 1，false - 0
	 * 
	 * @param key
	 * @param value
	 */
	...
	/**
	 * 设置�?�? boolean 值�?�实际写到配置中时，采用的是整数：true - 1，false - 0
	 * 
	 * @param key
	 * @param value
	 */
	...
	/**
	 * 获取�?个字符串，如果不存在，则返回 defaultValue
	 * 
	 * @param key
	 * @param defaultValue
	 * @return
	...
	/**
	 * 获取�?�? boolean，如果不存在或格式错误，则返�? false
	 * 
	 * @param key
	 * @return
	 */
	...
	/**
	 * 获取�?�? boolean，如果不存在或格式错误，则返�? defaultValue
	 * 
	 * @param key
	 * @param defaultValue
	 * @return
	...
	/**
	 * 获取�?�? double，如果不存在或格式错误，则返�? defaultValue
	 * 
	 * @param key
	 * @param defaultValue
	 * @return
	...
	/**
	 * 获取�?�? int，如果不存在或格式错误，则返�? defaultValue
	 * 
	 * @param key
	 * @param defaultValue
	 * @return
	...
	public static class Property
	{
		...
		private String m_contents = null;		// 行内�?
		private String m_key = null;			// key
		private String m_value = null;			// value
		private String m_comments = "";			// 行末的注�?
		...
		public void setValue(String value)
		{
			...
			{
				m_contents = String.format("%s = %s \t%s", m_key, m_value, m_comments);
			}
		}
